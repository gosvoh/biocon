...
import type { MediaAboutUs2024 } from "@/db/schema";
import {
  Button,
  Form,
  FormInstance,
...
const EditForm = ({
  ...
  form: FormInstance;
  data?: typeof MediaAboutUs2024.$inferInsert;
}) => (
  <Form<typeof MediaAboutUs2024.$inferInsert>
    labelCol={{ span: 8 }}
    wrapperCol={{ span: 16 }}
    form={form}
    initialValues={{
  ...
  >
    <Form.Item<typeof MediaAboutUs2024.$inferInsert>
      name="title"
      label="Title"
      rules={[{ required: true }]}
    >
...
export default function MediaAboutUsTable({
  ...
  data: (typeof MediaAboutUs2024.$inferSelect)[];
}) {
  const [modal, context] = Modal.useModal();
  const [form] = Form.useForm<typeof MediaAboutUs2024.$inferInsert>();
                ...
                modal.confirm({
                  ...
                  onOk: async () => {
                    modal.confirm({
                      ...
                      onOk: async () => {
                        try {
                          const val: typeof MediaAboutUs2024.$inferInsert & {
                            image: any;
